...
CREATE TABLE produto (
    ...
    categoria ENUM('Eletrônicos', 'Eletrodomésticos', 'Móveis', 'Roupas') NOT NULL,
    PRIMARY KEY(id)
);

INSERT INTO produto (nome, descricao, preco, estoque, categoria) 
VALUES
('Smartphone', 'Smartphone com 128GB de memória', 1999.99, 50, 'Eletrônicos'),
('Geladeira', 'Geladeira Frost Free 400L', 3299.00, 10, 'Eletrodomésticos'),
('Sofá 3 Lugares', 'Sofá confortável em tecido', 1599.90, 5, 'Móveis'),
('Camiseta Básica', 'Camiseta de algodão unissex', 49.90, 100, 'Roupas'),
('TV LED 50', 'TV LED 50 polegadas 4K', 2799.00, 15, 'Eletrônicos'),
('Notebook Gamer', 'Notebook com 16GB RAM e 512GB SSD', 5999.00, 8, 'Eletrônicos'),
('Máquina de Lavar', 'Máquina de lavar roupas 15kg', 2299.00, 12, 'Eletrodomésticos'),
('Cadeira de Escritório', 'Cadeira ergonômica para escritório', 799.99, 20, 'Móveis'),
('Calça Jeans', 'Calça jeans masculina', 129.90, 80, 'Roupas'),
('Drone', 'Drone com câmera 4K', 3599.90, 25, 'Eletrônicos'),
('Ventilador de Mesa', 'Ventilador de mesa silencioso', 199.90, 40, 'Eletrodomésticos'),
('Armário de Cozinha', 'Armário de cozinha 3 portas', 1099.00, 7, 'Móveis'),
('Casaco de Inverno', 'Casaco de inverno com capuz', 399.90, 50, 'Roupas');

CREATE OR REPLACE VIEW vw_roupas AS
SELECT id, nome, preco, estoque
FROM produto
WHERE categoria = 'Roupas';

CREATE OR REPLACE VIEW vw_estoque AS
SELECT id, nome, preco, estoque,  (preco * estoque) AS valor_total_estoque
FROM produto
WHERE preco >=1000
ORDER BY nome DESC;
...
-- Carrega os dados de um arquivo CSV para uma tabela MySQL.
LOAD DATA LOCAL INFILE 'd:/temp/BD1-A13-cidade.csv'

INTO TABLE cidade

FIELDS TERMINATED BY ',' 
-- Define que os campos no arquivo CSV são separados por vírgulas.

ENCLOSED BY '"'
-- Indica que os campos são opcionalmente delimitados por apóstrofos.

LINES TERMINATED BY '\n' 
-- Define que cada linha de dados no arquivo CSV termina com um caractere de nova linha.

IGNORE 1 ROWS;
-- Ignora a primeira linha do arquivo CSV, geralmente contendo os cabeçalhos das colunas.
...
LOAD DATA LOCAL INFILE 'd:/temp/BD2-A13-arquivo.csv' 
INTO TABLE usuario 
FIELDS TERMINATED BY ',' 
ENCLOSED BY '"' 
LINES TERMINATED BY '\n' 
...
/*
+----------+--------------+--------------+-------------+
| Criador  | Total Femeas | Total Machos | Total Geral |
+----------+--------------+--------------+-------------+
| Angelina |            4 |            2 |           6 |
| GladiMau |            0 |            0 |           0 |
| Pablitus |            3 |            8 |          11 |
...
+-------------------------------------------------+-------------+--------------------------+
| Qual SGBD você irá usar em seu próximo projeto? | MSSQLServer |                        3 |
| Qual SGBD você irá usar em seu próximo projeto? | MySQL       |                       23 |
| Qual SGBD você irá usar em seu próximo projeto? | Oracle      |                        7 |
| Qual SGBD você irá usar em seu próximo projeto? | PostgreSQL  |                        8 |
| Qual SGBD você prefere?                         | MSSQLServer |                        1 |
| Qual SGBD você prefere?                         | MySQL       |                        9 |
| Qual SGBD você prefere?                         | Oracle      |                        3 |
| Qual SGBD você prefere?                         | PostgreSQL  |                        3 |
+-------------------------------------------------+-------------+--------------------------+
...
+-------------------------------------------------+-------+--------+------------+-------------+
| Qual SGBD você irá usar em seu próximo projeto? |    23 |      7 |          8 |           3 |
| Qual SGBD você prefere?                         |     9 |      3 |          3 |           1 |
+-------------------------------------------------+-------+--------+------------+-------------+
2 rows in set (0.00 sec)
*/
